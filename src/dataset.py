...
class RadarCameraDataset(Dataset):
    ...
    """
    def __init__(self, data_dir: str, num_points: int = 4096, target_res: int = 512, split: str = "train", indices_path: Optional[str] = None):
        super().__init__()
        ...
        if indices_path is not None and os.path.exists(indices_path):
            ...
            self.file_names = [common_names[i] for i in indices]
        else:
            np.random.seed(42)
            ...
            if split == "train":
                self.file_names = [common_names[i] for i in indices[:split_idx]]
            else:
                self.file_names = [common_names[i] for i in indices[split_idx:]]
            
            # Save indices if path provided but didn't exist
            if indices_path is not None:
                os.makedirs(os.path.dirname(indices_path), exist_ok=True)
                np.save(indices_path, indices[:split_idx] if split == "train" else indices[split_idx:])
    ...
    def __getitem__(self, idx: int) -> dict:
        ...
        # 2. Process Radar
        # Check for pre-processed cartesian data first
        cached_rpc_path = os.path.join(self.data_dir, "conditions_cartesian", f"{base_name}.npy")
        if os.path.exists(cached_rpc_path):
            rpc_points = np.load(cached_rpc_path)
        else:
